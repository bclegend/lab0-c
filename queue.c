...
int q_ascend(struct list_head *head)
{
    ...
    element_t *next = list_entry(head->next->next, element_t, list);
    ...
    while (&next->list != head) {
        if (strcmp(next->value, curr->value) > 0) {
            ...
            curr = list_entry(curr->list.next, element_t, list);
        } else {
            list_del(&next->list);
...
int q_descend(struct list_head *head)
{
    ...
    element_t *prev = list_entry(head->prev->prev, element_t, list);
    element_t *curr = list_entry(head->prev, element_t, list);
    while (&prev->list != head) {
        if (strcmp(prev->value, curr->value) > 0) {
            prev = list_entry(prev->list.prev, element_t, list);
            curr = list_entry(curr->list.prev, element_t, list);
        } else {
            list_del(&prev->list);
            q_release_element(prev);
            prev = list_entry(curr->list.prev, element_t, list);
        }
    }
    return q_size(head);
}
...
int q_merge(struct list_head *head, bool descend)
{
    ...
    list_for_each_entry_safe (entry, safe, head, chain) {
        list_splice_init(entry->q, &new);
    }
    q_sort(&new, descend);
    list_splice_init(&new, list_first_entry(head, queue_contex_t, chain)->q);
    return q_size(head);
}
